...
![Merkle Tree](./images/data-structures/merkle_tree.png)

## Patricia 树概述

Patricia 树（也叫 Radix 树）是 n 叉树，与 Merkle 树不同，它用于数据存储而非验证。

简单来说，Patricia 树是一种树形数据结构，所有数据存储在叶子节点中，每个非叶子节点是一个唯一字符串的字符，用来标识数据。通过这个唯一字符串，我们可以通过字符节点进行导航，最终到达数据。因此，它在数据检索方面非常高效。

Patricia 树通过消除只有一个子节点的冗余节点，比传统的 Trie 结构更节省空间。它通过在键之间共享前缀来实现紧凑性。这意味着，多个键之间的公共前缀会被共享，从而减少整体存储需求。
...
![tries](./images/data-structures/tries.png)
...
![transaction-trie](./images/data-structures/transaction_trie.png)

每笔交易在交易 Trie 中有一个映射，其中键是交易的索引，值是交易 T。交易索引和交易本身都采用 RLP 编码。它们组成一个键值对，存储在 Trie 中：
\text{RLP}(\text{index}) \to \text{RLP}(T)
...
![eth-tries](./images/data-structures/eth_tries.png)
...
![verkle-tree](./images/data-structures/verkle_tree_structure.png)
...
为了使客户端保持无状态性（stateless），至关重要的一点是，在验证区块时，客户端不需要存储整个或前一个区块链的状态。传入的区块应能提供必要的数据，以便客户端验证该区块。这些额外的证明数据称为“证人”（witness），使得无状态客户端能够在没有完整状态的情况下验证数据。通过使用区块内的数据，客户端还应该能够随着每个传入区块的到来来维护/增长本地状态。通过这种方式，客户端能够保证当前区块（以及它验证的随后的区块）中的状态转换是正确的。但它并不能保证当前区块所引用的以前区块的状态是正确的，因为区块生产者可能会在一个无效或非标准的区块上构建。

Verkle 树被设计为在存储和通信成本方面更高效。对于 1000 个叶子/数据，一个二叉 Merkle 树大约需要 4MB 的证明数据，而 Verkle 树则将其减少到 150KB。如果我们将证明数据包含在区块中，它不会对区块大小产生太大影响，但它将使无状态客户端更高效和可扩展。通过这种方式，无状态客户端将能够信任计算结果，而无需存储整个状态。
